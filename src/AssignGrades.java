...
public class AssignGrades {
    public static void main(String[] args) {
        ...
        int i = userInput.nextInt();  int[] marks = new int[i];  String[] grade = new String[i];
        System.out.println("Please enter the marks ");  for (int x = 0; x < i; x++) {
            marks[x] = userInput.nextInt();
            highestScore = Math.max(highestScore, marks[x]);
        }
        for (int x = 0; x < i; x++) {
            if (marks[x] >= highestScore - 10) { grade[x] = "A";
